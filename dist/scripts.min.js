let pokemonRepository = (function () {
    ...
    return fetch(t.detailsUrl)
      ...
      .then(function (n) {
        (t.imageUrl = n.sprites.front_default),
          (t.height = n.height),
          (t.types = n.types.map((t) => t.type.name)),
          (t.abilities = n.abilities.map((t) => t.ability.name));
      })
      .catch(function (t) {
        console.error(t);
      });
  ...
  function p(t) {
    ...
    let e = $(".modal-body"),
      o = $(".modal-title");
    e.empty(), o.text(a(t.name));
    let i,
      l,
      p = $(
        "<p>Height: " +
          ((l = Math.floor((i = (n = t.height) / 10) / 0.0254)),
          `${i}m (${Math.floor(l / 12)}' ${l % 12}")`) +
          "</p>"
      ),
      r = $('<img class="pokemon-img" src="' + t.imageUrl + '" />'),
      s = $("<p>Types: " + t.types + "</p>"),
      c = $("<p>Abilities: " + t.abilities + "</p>");
    e.append(r), e.append(p), e.append(s), e.append(c);
  }
  return {
    add: e,
    getAll: o,
